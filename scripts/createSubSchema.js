...
connection.query("INSERT INTO "+dbConfig.database+"."+dbConfig.layout_paramType_restrictions+" (dropdownType, dropdownValue, dropdownRestrictions) VALUES " +
    ...
    "('ulListing', 'UL891', '{" +
        "\"systemType\": [\"208Y/120VAC - 3PH, 4W\", \"240VAC - 3PH, 3W\", \"380Y/220VAC - 3PH, 4W\", " +
            "\"400Y/230VAC - 3PH, 4W\", \"415Y/240VAC - 3PH, 4W\", \"480VAC - 3PH, 3W\", \"480Y/277V - 3PH, 4W\", " +
...
for (let i = 0; i < quoteBrkAccData.length; i++) {
    if (quoteBrkAccData[i].brkAccOpt.length != 0) {
        for (let j = 0; j < quoteBrkAccData[i].brkAccOpt.length; j++) {
            connection.query("INSERT INTO " + dbConfig.database + '.' + dbConfig.brkAcc_options_table + " (brkAcc, brkAccName, brkAccOpt) VALUES (?,?,?)", [quoteBrkAccData[i].brkAcc, quoteBrkAccData[i].brkAccName, quoteBrkAccData[i].brkAccOpt[j]]);
        }
    } else {
        connection.query("INSERT INTO " + dbConfig.database + '.' + dbConfig.brkAcc_options_table + " (brkAcc, brkAccName, brkAccOpt) VALUES (?,?,?)", [quoteBrkAccData[i].brkAcc, quoteBrkAccData[i].brkAccName, null]);
    }
}
